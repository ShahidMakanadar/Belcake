...
function ViewAllOrder() {
    ...
    const getCakeList = async () => {
        ...
        let result = await fetch(`http://localhost:10000/getViewAllOrders/${userAuth.email}`, {
            method: 'post',
            headers: {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    let result = await fetch(`http://localhost:10000/RemoveViewAllOrder/${id}/${userAuth._id}`, {
                        method: "DELETE",
                        headers: {
    ...
    const handleSubmitStatus = async (userId, orderId, OrderedCakeId) => {
        if (!selectedStatus) {
            ...
            setLoading3(true)
            let result = await fetch(`http://localhost:10000/updateOrderStatus/${userId}/${orderId}/${OrderedCakeId}`, {
                method: "PUT",
    ...
    const searchHandler = async (key) => {
        if (key) {
            try {
                const result = await fetch(`http://localhost:10000/searchViewOrder/${key}`);
