...
const OrderProcess = () => {
                    ...
                    if (result.isConfirmed) {
                        try {
                            if (paymentMethod === " currently Disabled") {
                                setLoading(true)
                                let getkey = await fetch('http://localhost:10000/getKey')
                                let OrderData = await fetch(`http://localhost:10000/checkout/${ProductId}`, {
                                    method: "POST",
                                    ...
                                    var options = {
                                        ...
                                        handler:  async(response)=>{
                                            ...
                                            if(razorpay_payment_id && razorpay_order_id && razorpay_signature){
                                                let responseVerification = await fetch('http://localhost:10000/Verification',{
                                                    method: 'post',
    ...
    const handleSavingOrder = async ( razorpay_payment_id, razorpay_order_id) => {
        ...
        setLoading(true)
        let result = await fetch(`http://localhost:10000/OrderPlace/${auth._id}/${ProductId}`, {
            method: 'post',
        ...
        if (result.JWT_Err) {
            ...
            let finalResult = await fetch(`http://localhost:10000/OrderPlaceOwner/${tempAuth._id}/${currentOrderId}`, {
                method: 'get'
            })
